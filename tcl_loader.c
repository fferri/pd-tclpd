...
extern int tclpd_do_load_lib(t_canvas *canvas, char *objectname) {
    ...
    if(sys_onloadlist(objectname)) {
        logpost(0, PD_VERBOSE, "tclpd loader: already loaded: %s", objectname);
        return 1;
    }

    /* try looking in the path for (objectname).(tcl) ... */
    if(sys_verbose)
        logpost(0, PD_VERBOSE, "tclpd loader: searching for %s in path...", objectname);
    ...
    if(sys_verbose)
        logpost(0, PD_VERBOSE, "tclpd loader: searching for %s in path...", filename);
    if ((fd = canvas_open(canvas, filename, ".tcl",
        dirbuf, &nameptr, MAXPDSTRING, 1)) >= 0)
            goto found;

    if(sys_verbose)
        logpost(0, PD_VERBOSE, "tclpd loader: found nothing!");
    return 0;

found:
    logpost(0, PD_VERBOSE, "tclpd loader: found!");
    close(fd);
    ...
    filename[MAXPDSTRING - 1] = 0;
    logpost(0, PD_VERBOSE, "tclpd loader: absolute path is %s", filename);

    int result;

    // create the required tcl namespace for the class
    logpost(0, PD_VERBOSE, "tclpd loader: init namespace for class %s", classname);
    ...
    logpost(0, PD_VERBOSE, "%s", buf);

    // load tcl external:
    logpost(0, PD_VERBOSE, "tclpd loader: loading tcl file %s", filename);
    ...
    logpost(0, PD_VERBOSE, "tclpd loader: calling setup function for %s", classname);
